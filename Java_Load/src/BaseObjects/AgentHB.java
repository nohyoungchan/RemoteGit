	    	  ...
	    	  while (!this.isInterrupted()) 
	    	  {
				   ...
				   switch (globalScenario) 
				   {
			         case "Load_LogInOnly_ByThread":
			        	 log.info("@" + i + "@ "+ username + ": Thread on=>Load_LogInOnly_ByThread");
			        	//agent is already logged in by this point on beforesuite();
			            break;
			         case "Load_ChangeState_ByThread":
			        	log.info("@" + i + "@ "+ username + ": Thread on=>Load_ChangeState_ByThread");
			     		wait(waitSec);
			         ...
			         case "Load_LogAndSignOut_ByThread":
			        	   log.info("@" + i + "@ "+ username + ": Thread on=>Load_LogAndSignOut_ByThread");
				        	wait(globalSec);
				        	signoutWebAgent();
				        	wait(10);
				        	logIntoWebAgent();
			         ...
			         case "Load_LogAndReLogInAfterMin_ByThread":
			        	    log.info("@" + i + "@ "+ username + ": Thread on=>Load_LogAndReLogInAfterMin_ByThread");
			         ...
			         case "Load_LogAndSignoutCloseAfterMin_ByThread":
			        	    log.info("@" + i + "@ "+ username + ": Thread on=>Load_LogAndSignoutCloseAfterMin_ByThread");
				        	boolExit = true;
				        	waitMin(globalMinToRelogIn);
				        	WaitUntil_StatusBecomesIdle();
				        	signoutWebAgent();
